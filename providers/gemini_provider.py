"""Google Gemini LLM 프로바이더 구현."""

import json
import logging
import time
from typing import Optional

from google import genai
from google.genai import types

from providers.base import LLMProvider

logger = logging.getLogger(__name__)

# 팩트시트 생성용 시스템 프롬프트
_FACTSHEET_SYSTEM_PROMPT = """당신은 도서 사실 검증 전문가입니다. Google 검색을 활용하여 주어진 도서에 대한 팩트시트를 작성하세요.

팩트시트에 반드시 포함할 항목:
1. **저자/출판사 정보**: 저자 이력, 출판 연도, 출판사
2. **주요 등장인물**: 이름, 역할, 관계
3. **챕터 구조**: 각 장의 제목과 핵심 내용 요약
4. **핵심 사건 순서**: 시간순으로 정리된 주요 플롯 포인트
5. **자주 인용되는 구절**: 유명 인용문이나 핵심 문장
6. **자주 혼동되는 항목**: 비슷한 이름의 인물, 헷갈리는 사건 등

중요 규칙:
- 확인할 수 없는 정보는 반드시 '미확인'으로 표기하세요.
- 추측으로 정보를 채우지 마세요.
- 마크다운 형식으로 작성하세요."""

# 검증 지침
_VERIFICATION_INSTRUCTION = """당신은 도서 독후감/독서 제출물의 사실 검증 전문가입니다.

## 팩트시트 (검증 기준)
{factsheet}

## 검증 규칙
1. 각 claim을 위 팩트시트의 내용과 대조하세요.
2. 팩트시트에 해당 내용이 있으면 '일치' 또는 '모순'으로 판정하세요.
3. **팩트시트에 해당 내용이 없으면, verdict는 반드시 '판단불가'로 하세요.**
4. **절대로 모델 자체의 지식으로 '모순'을 판정하지 마세요.** 오직 팩트시트 기준으로만 판단합니다.
5. 각 판정에 대해 팩트시트의 어느 부분을 근거로 했는지 명시하세요.

## 응답 형식 (반드시 아래 JSON 형식으로만 응답하세요)
{{
    "claims": [
        {{
            "claim": "검증 대상 주장",
            "verdict": "일치" | "모순" | "판단불가",
            "explanation": "판정 근거 설명",
            "factsheet_basis": "팩트시트에서 참조한 부분 (없으면 null)"
        }}
    ],
    "hallucination_score": 0-100,
    "overall": "전체 평가 요약",
    "interview_questions": ["면담 확인 질문1", "면담 확인 질문2", "면담 확인 질문3"]
}}"""


def _parse_json_safe(text: str) -> Optional[dict]:
    """JSON 파싱을 안전하게 시도. 코드 블록 마커도 처리."""
    if not text:
        return None
    cleaned = text.strip()
    if cleaned.startswith("```"):
        lines = cleaned.split("\n", 1)
        if len(lines) > 1:
            cleaned = lines[1]
        if cleaned.endswith("```"):
            cleaned = cleaned[:-3]
        cleaned = cleaned.strip()
    try:
        return json.loads(cleaned)
    except json.JSONDecodeError:
        return None


class GeminiProvider(LLMProvider):
    """Google Gemini API를 사용하는 프로바이더."""

    def __init__(self, api_key: str, model_screening: str, model_verify: str, cost_tracker: Optional[object] = None):
        super().__init__(api_key, model_screening, model_verify, cost_tracker)
        self.client = genai.Client(api_key=api_key)

    def _call_with_backoff(self, api_call, max_retries: int = 3, base_delay: float = 2.0):
        """429 에러 시 지수 백오프로 재시도."""
        for attempt in range(max_retries + 1):
            try:
                return api_call()
            except Exception as e:
                # google-genai SDK의 429 에러 확인
                error_str = str(e)
                is_rate_limit = "429" in error_str or "RESOURCE_EXHAUSTED" in error_str
                if is_rate_limit and attempt < max_retries:
                    delay = base_delay * (2 ** attempt)
                    logger.warning(f"Rate limit 발생, {delay}초 후 재시도 ({attempt + 1}/{max_retries})")
                    time.sleep(delay)
                else:
                    raise

    def screen(self, system_prompt: str, text: str, max_tokens: int = 1500) -> dict:
        """2단계: 제출물 AI 스크리닝.

        프롬프트에 JSON 출력 지시를 포함하고, 파싱 실패 시 1회 재시도.
        """
        try:
            # JSON 출력 지시를 시스템 프롬프트에 추가
            json_instruction = (
                "\n\n반드시 순수한 JSON 형식으로만 응답하세요. "
                "```json 마커나 다른 텍스트 없이 JSON 객체만 출력하세요."
            )
            full_system_prompt = system_prompt + json_instruction

            def _api_call():
                return self.client.models.generate_content(
                    model=self.model_screening,
                    contents=text,
                    config=types.GenerateContentConfig(
                        system_instruction=full_system_prompt,
                        max_output_tokens=max_tokens,
                    ),
                )

            response = self._call_with_backoff(_api_call)

            # 토큰 사용량 추적
            if response.usage_metadata:
                self._update_usage(
                    "screening",
                    input_tokens=response.usage_metadata.prompt_token_count or 0,
                    output_tokens=response.usage_metadata.candidates_token_count or 0,
                )

            # JSON 파싱 (실패 시 1회 재시도)
            raw_text = response.text or ""
            result = _parse_json_safe(raw_text)
            if result is not None:
                return result

            # 재시도
            logger.warning("JSON 파싱 실패, 재시도 중...")
            retry_system = system_prompt + (
                "\n\n중요: 반드시 유효한 JSON만 출력하세요. "
                "코드 블록 마커(```)나 설명 텍스트 없이 순수 JSON 객체만 응답하세요."
            )

            retry_response = self._call_with_backoff(lambda: self.client.models.generate_content(
                model=self.model_screening,
                contents=text,
                config=types.GenerateContentConfig(
                    system_instruction=retry_system,
                    max_output_tokens=max_tokens,
                ),
            ))

            # 재시도 토큰 사용량 추적
            if retry_response.usage_metadata:
                self._update_usage(
                    "screening",
                    input_tokens=retry_response.usage_metadata.prompt_token_count or 0,
                    output_tokens=retry_response.usage_metadata.candidates_token_count or 0,
                )

            retry_text = retry_response.text or ""
            retry_result = _parse_json_safe(retry_text)
            if retry_result is not None:
                return retry_result

            logger.error(f"JSON 파싱 재시도 실패. 원본 응답: {retry_text[:200]}")
            return {"error": "JSON 파싱 실패", "raw_response": retry_text[:500]}

        except Exception as e:
            logger.error(f"Gemini 스크리닝 중 에러: {e}")
            return {"error": str(e)}

    def generate_factsheet(self, book_title: str, max_tokens: int = 2000) -> str:
        """3-1단계: 웹 검색(Google Search 그라운딩) 기반 팩트시트 생성."""
        try:
            user_prompt = f"다음 도서에 대한 팩트시트를 작성해주세요: 《{book_title}》"

            # Google Search 그라운딩 도구 설정
            search_tool = types.Tool(google_search=types.GoogleSearch())

            def _api_call():
                return self.client.models.generate_content(
                    model=self.model_verify,
                    contents=user_prompt,
                    config=types.GenerateContentConfig(
                        system_instruction=_FACTSHEET_SYSTEM_PROMPT,
                        tools=[search_tool],
                        max_output_tokens=max_tokens,
                    ),
                )

            response = self._call_with_backoff(_api_call)

            # 토큰 사용량 추적
            if response.usage_metadata:
                self._update_usage(
                    "factsheet",
                    input_tokens=response.usage_metadata.prompt_token_count or 0,
                    output_tokens=response.usage_metadata.candidates_token_count or 0,
                )

            # 웹 검색 사용 여부 확인 및 카운트 증가
            # 그라운딩 메타데이터가 있으면 검색이 실행된 것
            if response.candidates:
                candidate = response.candidates[0]
                grounding_metadata = getattr(candidate, "grounding_metadata", None)
                if grounding_metadata:
                    self._add_web_search(1)

            # 텍스트 추출
            factsheet = response.text or ""

            if not factsheet.strip():
                logger.warning("팩트시트 생성 결과가 비어있음")
                return f"# {book_title} 팩트시트\n\n팩트시트 생성에 실패했습니다."

            return factsheet

        except Exception as e:
            logger.error(f"Gemini 팩트시트 생성 중 에러: {e}")
            return f"# {book_title} 팩트시트\n\n에러: {str(e)}"

    def verify_claims(self, system_prompt: str, claims: list, full_text: str, max_tokens: int = 2000) -> dict:
        """3-2단계: 팩트시트 기반 사실 검증."""
        try:
            # 사용자 메시지 구성
            claims_text = "\n".join(f"- {claim}" for claim in claims)
            user_message = (
                f"## 검증 대상 주장 목록\n{claims_text}\n\n"
                f"## 제출물 전문\n{full_text}"
            )

            def _api_call():
                return self.client.models.generate_content(
                    model=self.model_verify,
                    contents=user_message,
                    config=types.GenerateContentConfig(
                        system_instruction=system_prompt,
                        max_output_tokens=max_tokens,
                    ),
                )

            response = self._call_with_backoff(_api_call)

            # 토큰 사용량 추적
            if response.usage_metadata:
                self._update_usage(
                    "verify",
                    input_tokens=response.usage_metadata.prompt_token_count or 0,
                    output_tokens=response.usage_metadata.candidates_token_count or 0,
                )

            # JSON 파싱
            raw_text = response.text or ""
            result = _parse_json_safe(raw_text)
            if result is not None:
                return result

            logger.error(f"검증 결과 JSON 파싱 실패. 원본: {raw_text[:200]}")
            return {
                "error": "JSON 파싱 실패",
                "raw_response": raw_text[:500],
                "claims": [],
                "hallucination_score": -1,
                "overall": "검증 결과 파싱 실패",
                "interview_questions": [],
            }

        except Exception as e:
            logger.error(f"Gemini 검증 중 에러: {e}")
            return {"error": str(e)}

    @classmethod
    def list_available_models(cls, api_key: str) -> list[str]:
        """google-genai SDK를 사용하여 사용 가능한 Gemini 모델 목록을 동적으로 조회합니다."""
        try:
            from google import genai
            client = genai.Client(api_key=api_key)
            models_list = client.models.list()
            
            res = []
            for m in models_list:
                actions = getattr(m, 'supported_actions', [])
                if actions and any('generateContent' in action for action in actions):
                    name = m.name
                    if name.startswith("models/"):
                        name = name[len("models/"):]
                    res.append(name)
            return sorted(list(set(res)))
        except Exception as e:
            logger.error(f"Gemini 모델 동적 조회 중 오류 발생: {e}")
            raise

    @classmethod
    def build_verification_prompt(cls, factsheet: str) -> str:
        """팩트시트를 포함한 검증용 시스템 프롬프트 생성 헬퍼."""
        return _VERIFICATION_INSTRUCTION.format(factsheet=factsheet)
